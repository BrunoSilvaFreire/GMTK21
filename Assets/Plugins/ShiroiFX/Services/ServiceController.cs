...
namespace Shiroi.FX.Services {
    ...
    public abstract class ServiceController<T> : MonoBehaviour {
        ...
        private readonly List<Service> activeServices = new();
        private bool hadActiveServiceLastFrame;

        private void Update() {
            if (activeServices.IsEmpty()) {
                if (hadActiveServiceLastFrame) ResetState();
            ...
            if (activeServices.IsNullOrEmpty()) return;
            ...
            switch (BlendMode) {
                case ServiceBlendMode.Blend:
                    ...
                    break;
                default:
                    if (!(GetServiceToUse() is Service<T> s)) return;
        ...
        private static Service FindNewest(IEnumerable<Service> services) {
            ...
            foreach (var activeService in services) {
                ...
                if (timedService == null || timedService.TimeLeft < s.TimeLeft) timedService = s;
            }
        ...
        private Service GetServiceToUse() {
            switch (BlendMode) {
                ...
                case ServiceBlendMode.UseHighestPriorityThenNewest:
                    ...
                    if (services.Count == 0) return services.First();
