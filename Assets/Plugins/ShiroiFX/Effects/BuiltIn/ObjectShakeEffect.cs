...
namespace Shiroi.FX.Effects.BuiltIn {
    ...
    public class ObjectShakeEffect : Effect {
        ...
        public static Vector3 GetOffset(EffectContext context, float time, PingPongValues values, ShakeMode mode,
            ...
            direction.Normalize();
            if ((dimensions & ShakeDimension.FirstDimension) != ShakeDimension.FirstDimension) direction.x = 0;

            if ((dimensions & ShakeDimension.SecondDimension) != ShakeDimension.SecondDimension) direction.y = 0;

            if ((dimensions & ShakeDimension.ThirdDimension) != ShakeDimension.ThirdDimension) direction.z = 0;
        ...
        public struct PingPongValues {
            ...
            public void Affect(ref Vector3 direction) {
                if (x) direction.x = -direction.x;

                if (y) direction.y = -direction.y;

                if (z) direction.z = -direction.z;
            }
